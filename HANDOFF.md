...
> **分支说明（port/v141-on-main）：** 本分支已移除「APK 内置离线 rootfs 包」架构
> （ExtractActivity/offline-rootfs/两段式 CI 全部删除），恢复为在线分步安装
> （rootfs/Node/harness 多源下载），并合入了 v1.4.1 功能线（备份恢复修复、
> ANSI 终端、备份内容选择）。下文涉及离线包/解压页/两段 CI 的描述均已过时，
> 以当前代码为准。
